...
public class Risposta {
	...
	public String eseguiComando(Message messaggio)
	{
        ...
        switch (comando[0]) {
        	...
        	case ("/user"): {
        	  ...
        	  if (user.isLogged()) {
        		Main.log.warn(
        			messaggio.from().username() + "(" + messaggio.from().id() + ") ha tentato nuovamente l'accesso user.");
        		...
        		if (text.equalsIgnoreCase(Errors.USER_NOT_FOUND)) {
        		  Main.log
        			  .info("ACCESSO USER NEGATO A: " + messaggio.from().username() + " - id(" + messaggio.from().id() + ")");
        		} else {
        		  Main.log.info(
        			  "ACCESSO USER CONSENTITO A: " + messaggio.from().username() + " - id(" + messaggio.from().id() + ")");
        		}
        	  }
        	  return text;
        	}
        	case ("/userhelp"):
        	  ...
        	  if (!user.isLogged()) {
        		Main.log.info(messaggio.from().username() + "(" + messaggio.from().id()
        			+ ") ha tentato un comando da user non facendone parte");
        	...
        	case ("/userexit"):
        	  ...
        	  if (!user.isLogged()) {
        		Main.log.info(messaggio.from().username() + "(" + messaggio.from().id()
        			+ ") ha tentato l'uscita user non facendone parte");
        	...
        	case ("/mylady"): {
        	  Message message = null;
        	  if (mylady == true) {
        		Main.log.info(messaggio.from().firstName() + " " + messaggio.from().lastName()
        			+ " ha tentato nuovamente di effettuare l'accesso da mylady");
        	...
        	case ("/love"):
        	  if (mylady || controllaAdmin()) {
        		try {
        		  ...
        		  Main.log.error("Errore durante l'esecuzione di /love per l'utente " + messaggio.from().username(), e);
        	...
        	case ("/admin"): {
        	  ...
        	  if (controllaAdmin()) {
        		Main.log.warn(
        			messaggio.from().username() + "(" + messaggio.from().id() + ") ha tentato nuovamente l'accesso admin.");
        		text = Errors.ADMIN_ALREADY_LOGGED;
        	  } else {
        		if (user.isLogged()) {
        		  Main.log.warn(messaggio.from().username() + "(" + messaggio.from().id()
        			  + ") ha tentato l'accesso admin mentre loggato come user.");
        		  text = Errors.ADMIN_ALREADY_LOGGED;
        		} else {
        		  ...
        		  if (accesso) {
        			Main.log.warn(
        				"ACCESSO ADMIN CONSENTITO A: " + messaggio.from().username() + " - id(" + messaggio.from().id() + ")");
        			...
        			Main.log.warn(
        				"ACCESSO ADMIN NEGATO A: " + messaggio.from().username() + " - id(" + messaggio.from().id() + ")");
        	...
        	case ("/resettimeout"): {
        	  if (controllaAdmin()) {
        		Main.resetTimeoutRetry();
        		return "Timeout settato a 50 ms";
        	  } else {
        		return Errors.ADMIN_NOT_LOGGED;
        	  }
        	}
        	...
        	case ("/storygame"):
        	  ...
        	  try {
        		Sender.sendMessage(messaggio.from().id(), Messages.REQUEST_ID_TARGET);
        		String idTarget = Getter.attendiMessaggio(idthread);
        		if (idTarget != null && !idTarget.isEmpty() && idTarget.startsWith("/")) {
        		...
        		Sender.sendMessage(messaggio.from().id(), "L'utente ha rifiutato il tuo invito");
        	  } catch (NumberFormatException ex2) {
        		Sender.sendMessage(messaggio.from().id(), "ID inserito non valido");
        	  } finally {
        	...
        	case ("/foto"): {
        	  Message message = null;
        	  Sender.sendMessage(messaggio.from().id(), Messages.PHOTO_NEED_TYPE);
        	  // attendo categoria
        	  message = attendiMessaggio();
	...
	public String generaRisposta(Message messaggio) {
		...
		// eseguo comando
		if(testoMessaggio.substring(0,1).equalsIgnoreCase("/")) {
			return eseguiComando(messaggio);
		}
		
		risposta = FileUtils.leggiFilesRisposte(testoMessaggio);
		if(risposta != null) {
			return risposta;
		}
		...
		if(risposta == null) {
			try {
				if(!Costants.ENABLE_BOT_RESPONSE) {
					risposta = ANNULLA_SPEDIZIONE_MESSAGGIO;
				} else {
