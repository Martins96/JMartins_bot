...
public class CatGameMain extends GameBase{
	...
	private boolean enableFoto = true;
	...
	public String startGame() {
		...
		//richiesta di abilitare le foto
		while (!"/Si".equals(message) && !"/No".equals(message)) {
			sendMessage("Abilitare l'invio delle foto?\n/Si\n/No");
			message = getMessage();
		}
		//true se richisto si, altrimenti false
		enableFoto = "/Si".equals(message);
		...
		while (!exitGame) {
			...
			} catch (EndGameRequestException e) {
				//l'utente ha mandato /exit o /quit nel gioco, richiesto uscire
				break ingame;
			}
		}
